...
(function () {
  ...
  function AgentMark({ agent, size = 36, active = false }) {
    const initials = agent.id === 'Claude_Code' ? 'CC' : 'AG';
  ...
  function HandoffFlow({ animated = true, live }) {
    const HD = window.HD;
    const sender = window.HDLib.agentOf(live ? live.sender : 'Claude_Code');
    const receiver = window.HDLib.agentOf(live ? live.receiver : 'Github_Actions');
  ...
  window.HandoffFlow = HandoffFlow;
  window.AgentSummary = AgentSummary;
